...
void Ride:: operator = (const Ride &Rides){
    id= Rides.id;
     pickup= Rides.pickup;
    pickupTime= Rides.pickupTime;
    dropoff=Rides.dropoff;
    size=Rides.size; //size of party
    pets=Rides.pets;
    dropoffTime = Rides.dropoffTime;
    status=Rides.status; //active
    rating = Rides.rating;
}
...
void Ride :: PrintInfo(){
    ...
    cout <<"Size of size:" <<size<<endl;//must include for loop
    cout << "Includes Pets: " ;
    if(pets == false){
        cout << "No pets" <<endl;
    }
...
void Ride::printRides(){
    
}
void Ride:: updateRideStatus(){
